...
async function NowPlayingMovies({
  ...
  const [config, nowPlayingData] = await Promise.all([
    getTMDBConfig(),
    getNowPlayingMovies(currentPage),
  ]);

  // Limit total pages; TMDB may return impractical total_pages
  const totalPages = Math.min(nowPlayingData.total_pages, MAX_TMDB_PAGES);
  ...
  const movies = filterWithImages(nowPlayingData.results);
